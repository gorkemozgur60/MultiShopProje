...
namespace MultiShop.Catalog.Services.OfferDiscountServices
{
    public class OfferDiscountService : IOfferDiscountService
    {
        ...
        public async Task CreateOfferDiscountAsync(CreateOfferDiscount createOfferDiscountDto)
        {
        ...
        public async Task<List<ResultOfferDiscount>> GetAllOfferDiscountAsync()
        {
            var values = await _offerDiscountCollection.Find(x => true).ToListAsync();
            return _mapper.Map<List<ResultOfferDiscount>>(values);
        }

        public async Task<GetByOfferDiscount> GetByIdOfferDiscountAsync(string id)
        {
            var values = await _offerDiscountCollection.Find<OfferDiscount>(x => x.OfferDiscountId == id).FirstOrDefaultAsync();
            return _mapper.Map<GetByOfferDiscount>(values);
        }

        public async Task UpdateOfferDiscountAsync(UpdateOfferDiscount updateOfferDiscountDto)
        {
