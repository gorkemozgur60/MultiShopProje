...
namespace MultiShop.Catalog.Services.AboutServices
{
    public class AboutService : IAboutService
    {
        ...
        public async Task CreateAboutAsync(CreateAbout createAboutDto)
        {
        ...
        public async Task<List<ResultAbout>> GetAllAboutAsync()
        {
            var values = await _aboutCollection.Find(x => true).ToListAsync();
            return _mapper.Map<List<ResultAbout>>(values);
        }

        public async Task<GetByAbout> GetByIdAboutAsync(string id)
        {
            var values = await _aboutCollection.Find<About>(x => x.AboutId == id).FirstOrDefaultAsync();
            return _mapper.Map<GetByAbout>(values);
        }

        public async Task UpdateAboutAsync(UpdateAbout updateAboutDto)
        {
