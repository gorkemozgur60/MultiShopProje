...
namespace MultiShop.Catalog.Services.SpecialOfferServices
{
    public class SpecialOfferService : ISpecialOfferService
    {
        ...
        public async Task CreateSpecialOfferAsync(CreateSpecialOffer createSpecialOfferDto)
        {
        ...
        public async Task<List<ResultSpecialOffer>> GetAllSpecialOfferAsync()
        {
            var values = await _specialOfferCollection.Find(x=> true).ToListAsync();
            return _mapper.Map<List<ResultSpecialOffer>>(values);
        }

        public async Task<GetBySpecialOffer> GetByIdSpecialOfferAsync(string id)
        {
            var values = await _specialOfferCollection.Find<SpecialOffer>(x=> x.SpecialOfferId == id).FirstOrDefaultAsync();
            return _mapper.Map<GetBySpecialOffer>(values);
        }

        public async Task UpdateSpecialOfferAsync(UpdateSpecialOffer updateSpecialOfferDto)
        {
